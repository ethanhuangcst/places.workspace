...
**DoD:** binding checklist and honesty matrix live in [4.test-strategy.md](../../agent-specs/4.test-strategy.md) **§1.1 Vendor live DoD**. A vendor story is not done until those items pass and the operator has seen a real-pin result (not only `make test`). AC status must be `live-honest` / `fail-closed` / `fixture-only` — not `implemented`.
...
- [4.test-strategy.md](../../agent-specs/4.test-strategy.md) §1.1

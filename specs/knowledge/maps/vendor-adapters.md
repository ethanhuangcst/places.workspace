...
| **Auto-select** | ADR-052: 大陆 → AMAP only; **大陆以外**（香港、澳门、台湾、海外）→ Google only. Caller `providers[]` overrides. Discover **不得**把 AMAP-only 扩成双源；Stay/hotel 同一套 D2+D4。港/澳须先于大陆框检测，否则误走 AMAP。 |
...
| Implemented | places-agent [`places-agent/src/adapters/amap/`](../places-agent/src/adapters/amap/) — `config.ts`, `direct.ts`, `card-mapper.ts`, `keywords.ts`, `live.ts`. Fixture remains when `PLACES_VENDOR_MODE` is not `live`. |
...
| Implemented | places-agent [`places-agent/src/adapters/google/`](../places-agent/src/adapters/google/) — `direct.ts`, `mcp-client.ts`, `live.ts` (ADR-052 D5). |
...
| Implemented | places-agent [`places-agent/src/adapters/open-meteo/`](../../../places-agent/src/adapters/open-meteo/) — `config.ts`, `direct.ts`, `live.ts`. Fixture remains when `PLACES_VENDOR_MODE` is not `live`. |
...
| Implemented | places-agent [`places-agent/src/adapters/tripadvisor/`](../places-agent/src/adapters/tripadvisor/) — `config.ts`, `direct.ts`, `match.ts`, `card-mapper.ts`, `live.ts`. Fixture remains when `PLACES_VENDOR_MODE` is not `live`. Missing key in live → skip `errors.provider_unconfigured` ([ADR-021](../../adr/ADR-021-live-vendor-no-fixture.md)). |

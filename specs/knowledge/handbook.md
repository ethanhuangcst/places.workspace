...
**Delivery slices** follow agent capabilities (ADR-018), not “admin vs gateway vs intelligence.” All admin UI (Features 14–19) is MVP-1. Canonical table: `../agent-specs/agent-stories.md`.
...
Operator chrome is the kb.agent-mate.ai 性冷淡 family (logo on `#fafafa`, locale switcher `EN CN HK TW`). Visual notes: [`ui/agent-mate-admin-visual.md`](./ui/agent-mate-admin-visual.md). Pixel spec: `../agent-specs/3.ui-design.md`.
...
| what2eat LLM / DB | No product `OPENAI_*`. Postgres **`what2eat`** (ADR-023). Child plan: `../2eat-specs/2eat-deployment-plan.md` |
| where2play | Specs + mock-up only. Child plan: `../2play-specs/6.deployment-plan.md`. Persistence TBD. |
...
Parent: `~/code/places-workspace/` — umbrella **specs** only. Children are separate remotes, gitignored from parent: `0.1.sdd.sample`, `0.2.release-bot`, `places-agent`, `what2eat`, `where2play`.

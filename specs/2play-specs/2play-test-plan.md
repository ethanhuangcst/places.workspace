...
在 workspace **common-test-strategy** 基线上扩展 where2play 增量规则。不降低金字塔层级、不跳过 auth 测试、不削弱质量检查清单。格式对齐 what2eat [`2eat-test-plan.md`](../2eat-specs/2eat-test-plan.md)。
...
| Agent 行程用例 | [`../agent-specs/agent-stories.md`](../agent-specs/agent-stories.md)（`plan_itinerary`） |
| ADR itinerary | [ADR-008](../adr/ADR-008-itinerary-ownership.md) |
| ADR live | [ADR-021](../adr/ADR-021-live-vendor-no-fixture.md) |
| ADR HTTP chat | [ADR-020](../adr/ADR-020-http-only-chat-and-enrich.md) |
...
**批次状态：Done（as-built，2026-08-23）** — 与 [ADR-043](../adr/ADR-043-chatbox-mcp-and-cross-product-closure.md) 双通道表一致。Stories **31–33** 代码与契约测绿；live E2E opt-in。
...
实现后落在 `where2play/tests/` 与 `e2e/`。下列为**计划用例**（命名可微调，覆盖不得少）。
...
对齐 [`itinerary-design.md`](./itinerary-design.md) 与 agent [`performance.md`](../agent-specs/performance.md) 第十一节 §11.8。
...
**真源：** agent [`e2e-test.md`](../agent-specs/e2e-test.md) §5 + [`e2e-test-result/INDEX.md`](../agent-specs/e2e-test-result/INDEX.md) + 各城 `NN-city.md`。
...
```bash
# agent 基线（已有）
cd places-agent && python3 scripts/e2e-places-agent.py --all

# where2play parity（待实现）
cd where2play && python3 e2e/test_agent_parity_30.py [--only lisbon] [--compare]
```
...
绑定 [ADR-067](../adr/ADR-067-llm-driven-discovery-replaces-stops-pool.md)。配对 agent：`agent-discover-110c` / `110d` · [`agent-test-plan`](../agent-specs/agent-test-plan.md) §44。**MVP-T8 Done 2026-09-18。**
...
## §15 Fill stream resilience（P1–P3，2026-09-17 probe）

| ID | 层 | 断言 | 状态 |
| --- | --- | --- | --- |
| TC-FILL-01 | UI | `runFillFromSkeleton` 300s AbortController；abort 后 `planSubPhase=idle`、解锁日 tab | Done |
| TC-FILL-02 | UI | NDJSON stream 无 `done`/`error` 关闭 → `play.plan.assistant_fill_timeout`；保留部分行程 | Done |
| TC-FILL-03 | Unit | `skeleton_patched` 同 stop 最多 3 次后强制 `stopIndex++` | Done |
| TC-FILL-04 | i18n | `play.plan.assistant_fill_timeout` 四 locale | Done |

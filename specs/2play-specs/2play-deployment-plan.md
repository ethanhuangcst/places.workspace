...
**Consumer:** release-bot semi-auto release on **野草云3** (`svr_hk_vps_3`). Operators follow `0.2.release-bot/knowledge/03-semi-auto-release.md`. Family overview: [`../6.deployment-plan.md`](../6.deployment-plan.md). Sister plan: [`../2eat-specs/6.deployment-plan.md`](../2eat-specs/6.deployment-plan.md).

**App repo (target):** `ethanhuangcst/where2play.places` · local folder `where2play/`  
**Wave:** **Third** in the places family. **Upstream:** places-agent is **live** (`https://places.agent-mate.ai`, host debug `3007→3000`). Product gate: BFF uses `search_places` + `plan_itinerary` live-honest ([ADR-008](../adr/ADR-008-itinerary-ownership.md), [ADR-021](../adr/ADR-021-live-vendor-no-fixture.md)).  
...
| Artifact | Status |
| --- | --- |
| Next app + BFF (`package.json`) | **Blocker** — local `where2play/` is specs + `ui-mockup` only |
...
Thin app: **no** map vendor keys. Itinerary **engine** stays on places-agent ([ADR-008](../adr/ADR-008-itinerary-ownership.md)). Product **Qwen** (`QWEN_*`) on this BFF (ADR-047). Persistence: PostgreSQL **`where2play`** ([ADR-033](../adr/ADR-033-where2play-postgres-prisma.md)).
...
| `DATABASE_URL` | **yes** | Aliyun `…@101.132.156.250:5432/where2play` ([ADR-033](../adr/ADR-033-where2play-postgres-prisma.md)). Portainer secret. |
...
| Item | where2play |
| --- | --- |
| Engine | **PostgreSQL + Prisma** ([ADR-033](../adr/ADR-033-where2play-postgres-prisma.md)) |
...
- [ ] Signed-in trip / search path returns **live** place ids — no `fixture_` ([ADR-021](../adr/ADR-021-live-vendor-no-fixture.md))
...
| Doc | Role |
| --- | --- |
| [`../6.deployment-plan.md`](../6.deployment-plan.md) | Family plan |
| [`README.md`](./README.md) | Specs + mock-up |
| [`../2eat-specs/6.deployment-plan.md`](../2eat-specs/6.deployment-plan.md) | Sister thin-app plan (Postgres + no product `OPENAI_*`) |

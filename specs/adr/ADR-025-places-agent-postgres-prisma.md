...
2. **Local dev:** PostgreSQL. Dedicated databases `places_agent` and `places_agent_test`. Default: reuse the what2eat local instance on **`localhost:5435`**. Optional isolated compose: `places-agent/docker-compose.dev.yml` on **`5436→5432`**. Never store agent rows in the `what2eat` database.
...
- Existing local SQLite files under `places-agent/.data/*.db` are leftover; do not treat them as source of truth. Re-seed admin after first Postgres migrate.

...
2. **L1 不接 LLM**（与 [performance.md](../agent-specs/performance.md) 三层一致）。质量靠：热门城 **must-see seed query**、改进泛搜、硬过滤、seed-token 优先排序，再 cap。
...
5. discover 与 timed 共用并加硬 [`place-filters`](../../places-agent/src/core/place-filters.ts)（拒绝公司企业/停车场/公交站等）。

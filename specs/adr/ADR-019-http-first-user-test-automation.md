...
places-agent exposes the same tools on **HTTP** (`/v1/*`) and **MCP** (`/mcp`, `/sse`) per ADR-003. Feature 11 requires semantic parity across channels. [`8.user-test-cases.md`](../agent-specs/8.user-test-cases.md) defined manual **TC-H** (HTTP) and **TC-C** (ChatBox MCP) cases for release sign-off.
...
1. Automate **TC-H01–H15** exactly in [`tests/http-tc-h.test.ts`](../../places-agent/tests/http-tc-h.test.ts); include in default **`make test`** / PR gate.
...
- [`4.test-strategy.md`](../agent-specs/4.test-strategy.md) and [`8.user-test-cases.md`](../agent-specs/8.user-test-cases.md) must state HTTP automation + ChatBox pause.

...
3. **`make test-coverage`**: Vitest v8; include `src/**/*.{ts,tsx}`; overall statements/lines/functions/branches **≥ 80%**. Exclude list is explicit in [`../agent-specs/4.test-strategy.md`](../agent-specs/4.test-strategy.md) (admin UI via Playwright, Worker MCP client, env/`live` wiring, session/mail, NL loop). Core floors: `place-filters.ts` 100%; other listed core files **≥ 90% lines**.

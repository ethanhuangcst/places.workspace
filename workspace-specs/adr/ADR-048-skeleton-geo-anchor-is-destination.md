...
4. **2play intake** 用 `search_places`（query=用户输入，address=目的地）确认起点；禁止无城市酒店 geocode 作为 make 锚点。未命中则重输或忽略起点。供应商失败时只传 name。**命中后的卡字段与 fill 只抄：** [ADR-053](./ADR-053-origin-stay-as-stop-card.md)。
...
- 酒店地图钉：`skip` 或供应商失败时仍可仅 name；成功 `hit` 必须带卡上坐标（ADR-053）。

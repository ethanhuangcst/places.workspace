...
规范细节：[`1.places-agent/agent-specs/real-agent-refactory.md`](../../1.places-agent/agent-specs/real-agent-refactory.md)。  
...
what2eat 对 agent 的 HTTP 面保持：`geocode`、`search_restaurants`、`get_place_details`、`chat`。禁止将这些重指向 `plan_trip`，禁止因本 ADR 关闭 **places-agent** 上的 Qwen（2eat chat 工具环仍走 agent LLM）。行程工具与 Trip 账本对 2eat 不可见、不可强制。细则见 [`real-agent-refactory.md`](../../1.places-agent/agent-specs/real-agent-refactory.md)「what2eat 隔离」。
...
- [real-agent-refactory.md](../../1.places-agent/agent-specs/real-agent-refactory.md)
